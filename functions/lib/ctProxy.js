 ...
 */
function resolveTarget(baseUrl, path, method, query) {
    let base;
    try {
        base = new URL(baseUrl.trim());
    }
    catch {
        return { status: 400, error: 'Invalid X-CT-Base-URL' };
    }
    ...
    if (!route)
        return { status: 404, error: 'Unsupported ChurchTools route' };
    if (!route[1].includes(method))
        return { status: 405, error: 'Method not allowed' };
    const target = new URL(`/api${path}`, base.origin);
    target.search = query;
    return { url: target.toString() };
}
async function isSignedIn(idToken) {
    if (!idToken)
        return false;
    try {
...
//# sourceMappingURL=ctProxy.js.map